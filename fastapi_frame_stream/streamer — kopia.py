...
class FrameStreamer:
    ...
    def __init__(self):
        try:
            ...
            print("Error FrameStreamer:__init__.")
            pass
    ...
    async def send_frame(self, stream_id: str, frame: Union[str, UploadFile, bytes]) -> None:
        ...
        """
        print('\t send_frame  stream_id: ', stream_id)
        if isinstance(frame, str):
            print('\t send_frame      frame: ', frame)
            ...
            print('\t send_frame      _image_file_to_base64: ', frame)
            await self._store_image_str(stream_id, img_str)
        elif isinstance(frame, bytes):
            img_str = base64.b64encode(frame).decode("utf-8")
            print('\t send_frame      base64.b64encode(frame).decode("utf-8")  ', frame)
            await self._store_image_str(stream_id, img_str)


    async def send_frame_file(self, stream_id: str, file: str) -> None:
        """Send a frame to be streamed.
        ...
        # img_str = await self._image_file_to_base64(file)
        await self._store_image_str(stream_id, img_str)
        #img_stream_inv
        img_inv = cv2.bitwise_not(img)
        jpg_img_inv = cv2.imencode('.jpg', img_inv)
        img_str_inv = base64.b64encode(jpg_img_inv[1]).decode("utf-8")
        await self._store_image_str('img_stream_inv', img_str_inv)
